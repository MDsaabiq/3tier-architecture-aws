...
**Use your own S3 bucket to store code and copy it to web servers.**
...
```bash
mysql -h <your-db-endpoint> -u <db-username> -p
```
...
```bash
curl -o- https://raw.githubusercontent.com/<your-repo>/main/install.sh | bash
source ~/.bashrc
...
```bash
cd ~/
aws s3 cp s3://<your-bucket>/application-code/app-tier/ app-tier --recursive
...
Create an internal load balancer and update the Nginx configuration with the internal load balancer DNS name.

```text
internal-app-alb-<unique-id>.<region>.elb.amazonaws.com
```
...
cd ~/
aws s3 cp s3://<your-bucket>/application-code/web-tier/ web-tier --recursive
...
sudo rm nginx.conf
sudo aws s3 cp s3://<your-bucket>/application-code/nginx.conf .
...
## Generate an ACM certificate and add A/CNAME records
Once your certificate is validated, attach it to the public ALB and update your DNS records to access the application over HTTPS.
